...
% flipped so that largest amplitude goes in back of plot
files = flip(files);
...
for i = 1:length(files)
    ...
    wingbeat_count = 0;
    case_name = convertStringsToChars(case_name);
    speed = str2double(case_name(1));
    ...
    save(['Lift_PDMS_',num2str(speed),'Hz.mat'], 'filtered_lift_vals','wingbeats')
end
legend("Location","Southwest");
...
for i = 1:length(files)
    ...
    % Plot lift force
    plot(wingbeats, filtered_lift_vals, 'DisplayName', case_name, "LineWidth",3);
    save(['Lift_body_',num2str(speed),'Hz.mat'], 'filtered_lift_vals','wingbeats')
end
legend("Location","Southwest");
