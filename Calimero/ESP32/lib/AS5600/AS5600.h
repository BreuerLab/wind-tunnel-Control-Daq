...
//
//    FILE: AS5600.h
//  AUTHOR: Rob Tillaart, HACK Gwenael LAURENT
// VERSION: 0.3.1
// PURPOSE: Arduino library for AS5600 magnetic rotation meter
//    DATE: 2022-05-28, modif 2022-10-11
...
public:
  ...
  /***** DEBUT Ajout GL ******/
  //  Acquisition de l'angle avec fixedSamplingPeriod
  u_int16_t getAnglePos(bool fixedSamplingPeriod = true);

  //Position de l'angle dans les 4 quadrants
  u_int8_t getQuadrant(u_int16_t angle);

  //Calcul de la position absolue / angle total
  int32_t getTotalAngle(u_int8_t quadrantNumber, u_int16_t angle);

  //Retourne le nombre de tours
  int32_t getNumberofTurns();
  
  // Initialisation du quadrant de départ et du nombre de tours
  void initQuadrantAndTurns();

  // (Test) Mesure la durée de 1000 acquisitions
  void test_vitesse_1000_acquisitions();

  //  Gestion logicielle de l'offset RAW (0 à 4095)
  void setOffsetRAW(u_int16_t anglePos);
  u_int16_t getOffsetRAW();
  /***** FIN Ajout GL ******/
...
protected:
  ...
  // Gestion du temps entre chaque mesure d'angle
  unsigned long previousMicros = 0;
  unsigned long currentMicros = 0;
  unsigned long samplingPeriod = 250; // lecture toutes les 250 µs (Max Sampling rate du as5600 150µs ???)

  // pour le compteur de tours getTotalAngle()
  int8_t quadrantNumber;
  u_int8_t previousquadrantNumber = 1;
  int32_t numberofTurns = 0;
};
