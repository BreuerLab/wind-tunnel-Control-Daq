...
figure
histogram(tick_frame_pos, ticksPerRev)
...
time = results(:,1);
time_frame = time(laser_ind);
time_frame = time_frame(end-(num_images - 1):end);
norm_time = mod((time_frame - time_frame(1)) * wing_freq, 1);
...
% figure
% histogram(norm_signal, num_bins);
% xlabel("Encoder Ticks")
% ylabel("Frequency")

% Should show the same as the above plot
figure
bar(bin_count)
xlabel("Bin number", FontSize=16)
ylabel("Number of frames per bin", FontSize=16)
