...
PIV_plot(x(:,:,1), y(:,:,1), v_avg(:,:,1), params)
...
xlabel("y/c",FontSize=16)
ylabel("z/c",FontSize=16)
title("Streamwise vorticity, z = " + round(z(1,1,i)*1000,1) + " mm",FontSize=18)
...
f2 = figure;
% pcolor(D.x, D.y, vort_avg);
contourf(x(:,:,i), y(:,:,i), vort_x_avg(:,:,i), 71,'linestyle','none');
% xlim(xlims)
% ylim(ylims)
% ax = gca;
% shading(ax, 'interp');
...
xlabel("y/c",FontSize=16)
ylabel("z/c",FontSize=16)
title("Spanwise vorticity, z = " + round(z(1,1,i)*1000,1) + " mm",FontSize=18)
