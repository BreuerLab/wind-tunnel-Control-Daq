...
function [best_num_bins, best_bin_ind_arr, best_bin_count, best_bin_std] = findBestNumBins(norm_signal, bins_list, minFrames)
    ...
    for num_bins = bins_list
        ...
        for j = 1:num_bins
            ...
            % bin_std(j) = std(norm_signal(bin_indices));
            try
            bin_std(j) = range(norm_signal(bin_indices)) .* num_bins * 100;
            catch
            bin_std(j) = -1;
            end
        end
        ...
        % at least minFrames images per bin
        % std of bin_std falls below minStd threshold
        % average std across all frames is minimized
