...
function [results] = measure_force(obj, case_name, session_duration, offsets)
    ...
    % Déduire min/max sur ton enregistrement
    Vmin = min(analog_voltage);
    Vmax = max(analog_voltage);
        
    % Clamp entre Vmin et Vmax pour éviter valeurs aberrantes
    analog_voltage = max(min(analog_voltage, Vmax), Vmin);
    
    % Mise à l’échelle linéaire vers [0°, 360°]
    angle_per_turn = 360;  
    position_vals = (analog_voltage - Vmin) / (Vmax - Vmin) * angle_per_turn;
    
    % === Étape 2 : Unwrap pour avoir l'angle absolu cumulé ===
    position_vals_rad = deg2rad(position_vals);
    position_vals_unwrapped_rad = unwrap(position_vals_rad);
    position_vals_absolute = rad2deg(position_vals_unwrapped_rad);  % ← final


    % === Conversion de l'angle absolu en position Z via la loi géométrique ===
    
    angle_per_turn = 360;
    theta = deg2rad(mod(position_vals_absolute, angle_per_turn));  % angle dans un tour [0, 2π] pour la géométrie
    
    % Paramètres géométriques (à adapter selon ton montage)
    r = 9.6;    % rayon de l'excentrique (en mm)
    d = 20;   % distance horizontale entre centre de rotation et axe de coulisse (en mm)
    L = 200;  % longueur totale du bras/coulisse (en mm)
    
    % Calcul de la position verticale Z (projection du bras/coulisse)
    numerateur = r * sin(theta);
    denominateur = sqrt((r * cos(theta) - d).^2 + (r * sin(theta)).^2);
    Z = L * (numerateur ./ denominateur);  % position en mm (ou unités cohérentes)
...
function plot_results(obj, results, case_name, drift)
    ...
    % Trouver le premier indice où la variation par rapport à la première valeur dépasse 10 degrés
    angle_threshold = 15;  % seuil en degrés
    ...
    % Paramètres
    step_duration = 5;            % Durée d’un palier de fréquence (s)
    freq_start = 60;               % Hz
    freq_step = 10;
