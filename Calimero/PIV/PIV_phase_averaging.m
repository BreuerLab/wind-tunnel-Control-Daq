...
u_phase_avg = zeros(size(D.x,1), size(D.x,2), num_bins);
v_phase_avg = zeros(size(D.x,1), size(D.x,2), num_bins);
w_phase_avg = zeros(size(D.x,1), size(D.x,2), num_bins);
...
folder_name = PIV_case_name + "\vort\";
...
fps = 5;
gif_name = "anim";
export_plot_gifs(save_filepath + PIV_case_name + "\vort\", gif_name, fps)
