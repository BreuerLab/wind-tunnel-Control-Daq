...
% Compute vorticity
out1.vortZRaw = calculateVorticity(out1.xRaw, out1.yRaw, out1.uRaw, out1.vRaw);
...
function omega_x = calculateVorticityX(yRaw, zRaw, vRaw, wRaw)

% A: ALGORITHM TAKEN FROM RAFFEL'S PIV HANDBOOK
% Adapted for omega_x = dw/dy - dv/dz

omega_x = nan(size(vRaw));

dy = abs(yRaw(1,1,1) - yRaw(2,1,1));
dz = abs(zRaw(1,1,1) - zRaw(1,1,2));

% Do not compute edge vorticity
for k = 1:size(yRaw,1)      % x-direction (pass-through)
    for i = 2:size(yRaw,2)-1  % y-direction
        for j = 2:size(yRaw,3)-1 % z-direction
...
% 
% % Do not compute edge vorticity
% for k = 2:size(vRaw,3)-1      % z-direction
%     for i = 2:size(vRaw,1)-1  % y-direction
%         for j = 1:size(vRaw,2) % x-direction (pass-through)
% 
%             omega_x(i,j,k) = ...
%                 ( ...
%                 - (1/2)*dy*(wRaw(i-1,j,k-1) + 2*wRaw(i,j,k-1) + wRaw(i+1,j,k-1)) ...
%                 - (1/2)*dz*(vRaw(i+1,j,k-1) + 2*vRaw(i+1,j,k) + vRaw(i+1,j,k+1)) ...
%                 + (1/2)*dy*(wRaw(i+1,j,k+1) + 2*wRaw(i,j,k+1) + wRaw(i-1,j,k+1)) ...
%                 + (1/2)*dz*(vRaw(i-1,j,k+1) + 2*vRaw(i-1,j,k) + vRaw(i-1,j,k-1)) ...
%                 ) ...
%                 / (4*dy*dz);
