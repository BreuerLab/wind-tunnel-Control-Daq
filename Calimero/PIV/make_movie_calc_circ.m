function make_movie_calc_circ(x, y, u, v, Q, params)
    ...
    for k = 1:params.num_bins
        ...
        for j = 1:size(pts,2)
            ...
            u_cur = (ub_tr(j+1) + ub_tr(j))/2;
            v_cur = (vb_tr(j+1) + vb_tr(j))/2;
        ...
        % colormap(ax, jet);
        % colorbar;
        title(params.title + "Circ = " + circ(k));
