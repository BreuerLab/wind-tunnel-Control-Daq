function raw_extra_plot(f, tiles, time, extra_data, case_name, rate, fc, titles)
    ...
    extra_means = round(mean(filtered_extra_data, 2), 3);
    extra_SDs = round(std(filtered_extra_data, 0, 2), 3);
    extra_maxs = round(max(filtered_extra_data, [], 2), 3);
    extra_mins = round(min(filtered_extra_data, [], 2), 3);
