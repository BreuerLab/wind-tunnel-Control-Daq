...
function [wingbeat_forces, frames, wingbeat_avg_forces, wingbeat_SD_forces,...
    ...
    = wingbeat_transformation(num_wingbeats, forces, enc_pulse, AoA)

frames_per_beat = length(forces) / num_wingbeats;
...
long_rises = rise_idx(pulse_widths > mean(pulse_widths));

disp("Number of long rises: " + length(long_rises))
...
% wingbeat
forces = forces(:, round(frames_per_beat*(1/4)):end - round(frames_per_beat*(3/4)));
...
for j = 1:num_wingbeats
    for k = 1:round(frames_per_beat - 0.5)
        wingbeat_forces(:,j,k) = forces(:, k + round(frames_per_beat*(j-1)));
    end
