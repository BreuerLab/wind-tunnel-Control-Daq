...
wingbeat_pos_time = zeros(num_wingbeats, 18); % 18 pulses per wingbeat
...
for j = 1:num_wingbeats
    if (j == num_wingbeats)
        short_pulses = rise_idx(rise_idx >= cur_idx & rise_idx < length(mod_results));
        wingbeat_pos_time(j,:) = short_pulses;
        cur_wingbeat = mod_results(:, cur_idx:end);
        ...
        short_pulses = rise_idx(rise_idx >= cur_idx & rise_idx < end_idx);
        wingbeat_pos_time(j,:) = short_pulses;
        cur_wingbeat = mod_results(:, cur_idx:end_idx);
        cur_wingbeat_resampled = resample(cur_wingbeat.', frames_per_beat, length(cur_wingbeat)).';
        wingbeat_forces(:,j,:) = cur_wingbeat_resampled;
    end
...
wingbeat_pos_time = wingbeat_pos_time / rate;

pos = 0:1:17;
pos = pos / 18;
pos = 2*pi*pos;
pos = repmat(pos, 1, num_wingbeats);
pos = unwrap(pos) / (2*pi);

wingbeat_pos_time_arr = reshape(wingbeat_pos_time.', [], 1);

figure
plot(wingbeat_pos_time_arr, pos)
xlabel("Time (seconds)")
ylabel("Revolutions")

wingbeat_vel_time_arr = gradient(pos, wingbeat_pos_time_arr);

figure
plot(wingbeat_pos_time_arr, wingbeat_vel_time_arr)
xlabel("Time (seconds)")
