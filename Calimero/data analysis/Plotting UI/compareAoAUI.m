...
methods
    function obj = compareAoAUI(mon_num, data_path)
        ...
        obj.range = [-16 24];
        % obj.range = [10 16]; % CHANGE FOR DIFFERENT ANGLES
        ...
        for i = 1:length(cur_bird.uniq_list)
            ...
            if (isempty(amps) || sum(amps == amp) == 0)
                amps = [amps amp];
            end
        end
    ...
    function dynamic_plotting(obj)
        ...
        drop_y22 = drop_y2 - (unit_height + unit_spacing);
        d22 = uidropdown(option_panel);
        d22.Position = [10 drop_y22 180 30];
        d22.Items = string(obj.sel_bird.amps);
        d22.ValueChangedFcn = @(src, event) amp_change(src, event);

        % Dropdown box for wingbeat frequency selection
        drop_y3 = drop_y22 - (unit_height + unit_spacing);
        ...
        % s.MajorTicks = [10 12 14 16];
        % s.MinorTicks = [11 13 15];
...
methods (Access = private)
    function update_plot(obj, plot_panel)
        ...
        y_labels = [y_label_F, y_label_F, y_label_F, y_label_M, y_label_M, y_label_M, "Voltage (V)", "Current (mA)", "Power (W)"];
        ...
        if (obj.index == 0)
            ...
            for i = 1:length(obj.selection)
                ...
                for j = 1:length(struct_matches)
                    ...
                    % Found no matches, report error
                    if j == length(struct_matches)
                        error("Oops, didn't finding a matching structure selector")
                    end
                end
                ...
                if obj.driftData
                    ...
                    lim_drift = drift_vals(:,cur_bird.angles >= obj.range(1) & cur_bird.angles <= obj.range(2),:);
                else
                ...
                hold(ax, 'on');
                if obj.driftData
                    e = scatter(ax, lim_AoA_sel, lim_drift(idx,:,freq_index));
                    e.SizeData = 40;
        ...
        % -----------------------------------------------------
            ...
            for i = 1:length(obj.selection)
                ...
                if (idx == 9)
                    y_var = lim_avg_forces(idx-1,:,freq_index).*lim_avg_forces(idx-2,:,freq_index)/1000;
                    y_err = zeros(size(y_var));
                else
                    y_var = lim_avg_forces(idx,:,freq_index);
                    y_err = zeros(size(y_var));
                end
                e = errorbar(ax, lim_AoA_sel, y_var, y_err,'.');
