...
offsets_path = [];
% path to where experiment params files (.mat files) are stored
params_path = [];
% ----------------------------------------------
...
for i = 3:(length(dir_names))
    ...
    str = cur_name_parts{1};
    str = strtrim(strrep(str,'_',' '));
    tokens = regexp(str, '^(.*?)(\d+)$', 'tokens', 'once');

    cur_type = strtrim(tokens{1});   % "wings"
    cur_amp   = str2double(tokens{2}); % 10

    if (sum(type_list == cur_type) > 0 && sum(wing_amp_sel == cur_amp) > 0) % find matches
        ...
        params_path = [params_path filepath + "/raw data/"];
    end
end
...
% Get a list of all experment params files in the folder with the desired file name pattern.
filePattern = fullfile(params_path, '*.mat'); % Change to whatever pattern you need.
params_files = [];
for i = 1:length(filePattern)
    params_files = [params_files; dir(filePattern(i))];
end
...
for i = 1:2
    ...
    get_data_AoA(selected_vars, processed_files, offsets_files, norm_bool, sub_strings, shift_bool, sub_drift_bool, config_idx, num_config, freq_vals);
