clear all
%Forcedata=csvread('../Results/Data_Gravity/gravity1_data');
load('../Data/Data_Gravity/gravity1_data.mat');% load your fitting file

x = Save_avg(:,7);
y1 = Save_avg(:,3);
%% polynomials fit
[polypara, poly_error] = polyfit(x/10000,y1,2);
x_hat = -linspace(5100,5300,1000);
y_hat = polyval(polypara,x_hat);
[pks,locs] = findpeaks(-y_hat);
pos0 = x_hat(locs)*10000;
figure
hold on
plot(x,y1,'bo');
plot(x_hat*10000,y_hat,'r-');
zeroplot = plot(pos0,y_hat(locs),'r+','LineWidth',2.0);
datatip(zeroplot,pos0,y_hat(locs));
xlabel('encoder counts');
ylabel('z-axial force, [N]');
hold off
%% sine fit
[SineParams]=sineFit(x,y1); % Copyright (c) 2020, Peter Seibold
