...
methods
    ...
    function obj = compareWingbeatUI(mon_num, data_path)
        ...
        obj.mod_tot_inertial = false;
        obj.mod_tot_added_mass = false;
        obj.mod_tot_aero = false;
        obj.mod_tot_vibe = false;
    ...
    function dynamic_plotting(obj)
        ...
        c1b = uicheckbox(model_panel);
        c1b.Text = "";
        c1b.Position = [10 check1_y 20 unit_height];
        c1b.ValueChangedFcn = @(src, event) mod_tot_inertial_change(src, event, plot_panel);
        ...
        c2b = uicheckbox(model_panel);
        c2b.Text = "";
        c2b.Position = [10 check2_y 20 unit_height];
        c2b.ValueChangedFcn = @(src, event) mod_tot_added_mass_change(src, event, plot_panel);
        ...
        c3b = uicheckbox(model_panel);
        c3b.Text = "";
        c3b.Position = [10 check3_y 20 unit_height];
        c3b.ValueChangedFcn = @(src, event) mod_tot_aero_change(src, event, plot_panel);
        ...
        c4b = uicheckbox(model_panel);
        c4b.Text = "";
        c4b.Position = [10 check4_y 20 unit_height];
        c4b.ValueChangedFcn = @(src, event) mod_tot_vibe_change(src, event, plot_panel);
        ...
        function mod_tot_inertial_change(src, ~, plot_panel)
            if (src.Value)
                obj.mod_tot_inertial = true;
            else
                obj.mod_tot_inertial = false;
            end
            obj.update_plot(plot_panel);
        ...
        function mod_tot_added_mass_change(src, ~, plot_panel)
            if (src.Value)
                obj.mod_tot_added_mass = true;
            else
                obj.mod_tot_added_mass = false;
            end
            obj.update_plot(plot_panel);
        ...
        function mod_tot_aero_change(src, ~, plot_panel)
            if (src.Value)
                obj.mod_tot_aero = true;
            else
                obj.mod_tot_aero = false;
            end
            obj.update_plot(plot_panel);
        ...
        function mod_tot_vibe_change(src, ~, plot_panel)
            if (src.Value)
                obj.mod_tot_vibe = true;
            else
                obj.mod_tot_vibe = false;
            end
            obj.update_plot(plot_panel);
...
methods (Access = private)
    ...
    function update_plot(obj, plot_panel)
        ...
        % -----------------------------------------------------
            ...
            % the code below to handle subtraction may no longer work
            if (obj.sub)
            ...
            % fs = 9000;
            % fpass = [15 19];
            % cycle_avg_forces(idx, :) = bandstop(cycle_avg_forces(idx, :), fpass, fs,...
            %                     Steepness=[0.95 0.95], StopbandAttenuation=100);
    ...
    function [frames, cycle_avg_forces, upper_results, lower_results,...
              time, inertial_force, added_mass_force, aero_force, impulse_force, total_force] = ...
        ...
        [time, inertial_force, added_mass_force, aero_force, impulse_force] = ...
        ...
        % % History of other forces affecting impulse response
        % I = 0.0001; % amplitude of curve
        % phi = 0.7; % phase shift of curve
        % % Original values without convolution:
        ...
        % 
        % drag_force = impulse_force * sind(sel_angle);
        % lift_force = impulse_force * cosd(sel_angle);
        ...
        if (obj.norm)
            ...
            impulse_force = [impulse_force(:,1) / norm_factors(1),...
                    impulse_force(:,2) / norm_factors(1),...
                    impulse_force(:,3) / norm_factors(2)];
        else
