...
function [drift] = get_drift(experiment_filename, offsets_path)
    ...
    offsets_cur = readmatrix(offsets_path + offsets_filename);
    ...
    && strcmp(type, type_exp))
        ...
        for m = 1 : length(theFiles)
            time_baseFileName = theFiles(m).name;
    ...
    offsets_first = readmatrix(offsets_path + offsets_filename);
