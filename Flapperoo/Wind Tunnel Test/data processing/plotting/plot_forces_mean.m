function plot_forces_mean(frames, mean_results, upper_results, lower_results, case_name, subtitle, axes_labels)
    if (ndims(mean_results) == 2)
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];         
        yconf = [upper_results(2, :), lower_results(2, end:-1:1)];
        p = fill(xconf, yconf, 'blue');
        p.FaceColor = [0.8 0.8 1];      
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];         
        yconf = [upper_results(4, :), lower_results(4, end:-1:1)];
        p = fill(xconf, yconf, 'blue');
        p.FaceColor = [0.8 0.8 1];      
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];         
        yconf = [upper_results(6, :), lower_results(6, end:-1:1)];
        p = fill(xconf, yconf, 'blue');
        p.FaceColor = [0.8 0.8 1];      
        p.EdgeColor = 'none';
    ...
    elseif (ndims(mean_results) == 3)
        ...
        for i = 1:length(mean_results(1,1,:))
            xconf = [frames, frames(end:-1:1)];         
            yconf = [upper_results(2, :, i), lower_results(2, end:-1:1, i)];
            p = fill(xconf, yconf, colors(i,:),'HandleVisibility','off');
            p.FaceAlpha = 0.2;      
            p.EdgeColor = 'none';
        ...
        for i = 1:length(mean_results(1,1,:))
            xconf = [frames, frames(end:-1:1)];         
            yconf = [upper_results(4, :, i), lower_results(4, end:-1:1, i)];
            p = fill(xconf, yconf, colors(i,:),'HandleVisibility','off');
            p.FaceAlpha = 0.2;      
            p.EdgeColor = 'none';
        ...
        for i = 1:length(mean_results(1,1,:))
            xconf = [frames, frames(end:-1:1)];         
            yconf = [upper_results(6, :, i), lower_results(6, end:-1:1, i)];
            p = fill(xconf, yconf, colors(i,:),'HandleVisibility','off');
            p.FaceAlpha = 0.2;      
            p.EdgeColor = 'none';
