function plot_forces_mult(path, cases, main_title, sub_title)
    ...
    for i = 1:length(cases)
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];           
        yconf = [wingbeat_avg_forces(2, :) + wingbeat_std_forces(2, :), wingbeat_avg_forces(2, end:-1:1) - wingbeat_std_forces(2, end:-1:1)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];           
        yconf = [wingbeat_avg_forces(4, :) + wingbeat_std_forces(4, :), wingbeat_avg_forces(4, end:-1:1) - wingbeat_std_forces(4, end:-1:1)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames, frames(end:-1:1)];            
        yconf = [wingbeat_avg_forces(6, :) + wingbeat_std_forces(6, :), wingbeat_avg_forces(6, end:-1:1) - wingbeat_std_forces(6, end:-1:1)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
