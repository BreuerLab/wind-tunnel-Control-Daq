function plot_forces_mult(path, cases, main_title, sub_title)
    ...
    for i = 1:length(cases)
        ...
        hold on
        xconf = [frames; frames(end:-1:1)];           
        yconf = [wingbeat_avg_forces(:, 2) + wingbeat_std_forces(:, 2); wingbeat_avg_forces(end:-1:1, 2) - wingbeat_std_forces(end:-1:1, 2)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        plot(frames, wingbeat_avg_forces(:, 2), DisplayName=cases(i), Color=colors(i,:));
        hold off
        ...
        hold on
        xconf = [frames; frames(end:-1:1)];            
        yconf = [wingbeat_avg_forces(:, 3) + wingbeat_std_forces(:, 3); wingbeat_avg_forces(end:-1:1, 3) - wingbeat_std_forces(end:-1:1, 3)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames; frames(end:-1:1)];           
        yconf = [wingbeat_avg_forces(:, 4) + wingbeat_std_forces(:, 4); wingbeat_avg_forces(end:-1:1, 4) - wingbeat_std_forces(end:-1:1, 4)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        plot(frames, wingbeat_avg_forces(:, 4), DisplayName=cases(i), Color=colors(i,:));
        hold off
        title(["M_x (roll)"]);
        xlabel(axes_labels(1));
        ...
        hold on
        xconf = [frames; frames(end:-1:1)];            
        yconf = [wingbeat_avg_forces(:, 5) + wingbeat_std_forces(:, 5); wingbeat_avg_forces(end:-1:1, 5) - wingbeat_std_forces(end:-1:1, 5)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        ...
        hold on
        xconf = [frames; frames(end:-1:1)];            
        yconf = [wingbeat_avg_forces(:, 6) + wingbeat_std_forces(:, 6); wingbeat_avg_forces(end:-1:1, 6) - wingbeat_std_forces(end:-1:1, 6)];
        p = fill(xconf, yconf, colors(i,:), 'FaceAlpha',alpha,'HandleVisibility','off');  
        p.EdgeColor = 'none';
        plot(frames, wingbeat_avg_forces(:, 6), DisplayName=cases(i), Color=colors(i,:));
        hold off
        title(["M_z (yaw)"]);
        xlabel(axes_labels(1));
        ylabel(axes_labels(3));
