...
function this_DAQ = setup_DAQ(num_triggers, voltage, rate)
    ...
    count = 0;
    for i = 1:height(dev)
        curID = dev.DeviceID(i);
        if contains(curID, "Dev")
            daq_ID = curID;
            count = count + 1;
        end
        if count > 1
            error("Found " + count + " DAQs")
        end
    end

    disp("Connected to " + daq_ID)
    ...
    if (num_triggers >= 1)
        ...
        % ch6 = this_DAQ.addinput(daq_ID, "Port0/Line9", "Digital");
    end
    ...
    disp("The following channels have been stup on the DAQ")
    disp(this_DAQ.Channels)
end
...
function [offsets] = get_force_offsets(obj, case_name, tare_duration)
    ...
    trial_name = strjoin([case_name, "offsets", string(time_now)], "_");
...
function [results] = measure_force(obj, case_name, session_duration, offsets)
    ...
    time_now = datetime;
    time_now.Format = 'yyyy-MM-dd HH-mm-ss';
    trial_name = strjoin([case_name, "experiment", string(time_now)], "_");
...
function [f1, f2, f3] = plot_results(obj, results, case_name, drift, aliasing, f1, f2, f3, tiles_1, tiles_2, tiles_3)
    ...
    time_now = datetime;
    time_now.Format = 'yyyy-MM-dd HH-mm-ss';
    saveas(f1,'data\plots\' + case_name + "_" + string(time_now) + "_raw.jpg")
    ...
    for k = 1:6
        ...
        xlim([A_trimmed_time(1); A_trimmed_time(1) + 2])
    end

    % Label the whole figure.
    sgtitle({"Trimmed Force Transducer Data" strrep(case_name,'_','  ')});

    saveas(f2,'data\plots\' + case_name + "_" + string(time_now) + "_A_trimmed.jpg")
    ...
    saveas(f3,'data\plots\' + case_name + "_" + string(time_now) + "_A_trimmed_hist.jpg")
    ...
    saveas(f,'data\plots\' + case_name + "_" + string(time_now) + "_B_trimmed.jpg")
