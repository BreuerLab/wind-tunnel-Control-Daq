...
Date = datetime('today');
medium = 'water';
U = 0.3;      % flow velocity [m/s]
freq = 4/3*[0.2];%[0.2:0.2:0.6]; %[0.2:0.2:1]; %[0.2:0.1:1.0];     %[0.4,0.8]; %0.5;   %  [0.2,1,2];   %  0.5; % 1; %[0.5:0.1:1.0]; % % frequency [Hz]
A = 0.6;    % heaving amplitude normalized by chord
theta = -45; %-45; % % pitching amplitude [deg]
...
%     Properties = [char(Date),'_',medium,'_U',num2str(U,'%.1f'),'_f',num2str(f,'%.2f'),'_h',num2str(A),'_p',num2str(theta)];
      Properties = [char(date),'_water','_U',num2str(U,'%.1f'),'_f',num2str(f,'%.2f'),'_h',num2str(A),'_trapz',num2str(betah,'%.2f'), num2str(beta,'%.2f'),'_p',num2str(theta)]
          SaveFolder = ['C:\Users\ControlSystem\Documents\vertfoil\Experiments\1foil\Membrane\Membrane_heave_pitch\Blue50_50_thinner32pc_420micron_lambda0_1pt1\Trapezoidal\'];
            %SaveFolder = ['C:\Users\ControlSystem\Documents\vertfoil\Experiments\1foil\Membrane\Emptyframe_heave_pitch\Trapezoidal\wo_LE_TE\'];
%             SaveFolder = ['C:\Users\ControlSystem\Documents\vertfoil\Experiments\1foil\Membrane\Rigid_plate_no_LE_water_pitchheave\Trapezoidal\'];
...
%     [flume,out,dat,Prof2,Prof_out] = run_cycle_3rigs_20190408(f,0,0,0,0,theta,A,0,0,Ncycles,90);
    [flume,out,dat,Prof2,Prof_out,daq_timeStamps,daq_triggerTime] = run_cycle_3rigs_20190712_trapz_vm(f,0,0,0,0,theta,A,0,0,Ncycles,90);
