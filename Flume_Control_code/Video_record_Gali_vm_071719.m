...
freq = [0.1:0.1:2]; %[0.2:0.2:2]; %[0.4,0.8]; %1; % % frequency [Hz]
A = 0.5;    % heaving amplitude normalized by chord
...
%     fps = min(50,100*f);   % acquisition rate, frames per second
    fps = 60;   % acquisition rate, frames per second
...
%     SaveFolder = ['C:\Users\ControlSystem\Documents\vertfoil\Experiments\1foil\Membrane\Rigid_plate_no_LE_water_heave\',Properties,'_cam_fs',num2str(fps)];
    SaveFolder = ['C:\Users\ControlSystem\Documents\vertfoil\Experiments\1foil\Membrane\Membrane_Heave\In_water\BlueSilicone_Fast50_Slow50_lambda1p1'];
...
%     [flume,out,dat,Prof2,Prof_out,daq_timeStamps,daq_triggerTime] = run_cycle_3rigs_20190510(f,0,0,0,0,theta,A,0,0,Ncycles,90);
    [flume,out,dat,Prof2,Prof_out] = run_cycle_3rigs_20190717(f,0,0,0,0,theta,A,0,0,Ncycles,90);
...
%     cd ..
    save([Properties,'.mat']);
    h = warndlg('Press OK after saving is complete.');
    uiwait(h);
%     save 'CamData.mat';
%     clear data time
%     
