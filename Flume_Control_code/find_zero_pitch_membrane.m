function find_zero_pitch_membrane
% Finds the minimum lift on wallace hydrofoil while under flow. Slow, small angle
% variations for sensitive membrane foil
global pitch_bias
disp('finding zero.  Wallace will now move +/- 1 degrees')
move_new_pos_3rigs([0 0 0 0 1 0],2);
[out,prof] = move_new_pos_3rigs([0 0 0 0 -1 0],20);
...
[out,prof2] = move_new_pos_3rigs([0 0 0 0 1 0],20);
...
if round(f(2)) < 0 || round(f(2)) > numel(prof(:,5))
    ...
    [out,prof] = move_new_pos_3rigs([0 0 0 0 -5 0],20);
    ...
    plot(1:numel(prof(:,1)),prof(:,5),1:numel(out(:,8)),smooth(Lift,100),1:numel(Lift),((1:numel(Lift))-f(2))./f(1))
    hold on
        [out,prof2] = move_new_pos_3rigs([0 0 0 0 5 0],20);
