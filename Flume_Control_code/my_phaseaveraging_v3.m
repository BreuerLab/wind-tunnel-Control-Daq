function [out] = my_phaseaveraging_v3(filename, Nbins, PLOT, nb_steps)
%% Kyohei Onoue
%% 03152017
...
points_per_steps = 1371.44 ; 
endingstep = floor(points_per_steps*nb_steps) ;
...
ux1 = flume.foil3.Ux(1:ds:endingstep-1); % Vectrino's streamwise velocity (is this correct?)
uy1 = flume.foil3.Uy(1:ds:endingstep-1); % Vectrino's flow-normal velocity
uz1 = flume.foil3.Uz(1:ds:endingstep-1); % Vectrino's out-of-plane velocity
t = flume.foil3.t_sec(1:ds:endingstep); % time in sec.
% Filter drop-outs
%k = 1; % drop-outs are typically one single point
ux = hampel(ux1);
uy = hampel(uy1);
uz = hampel(uz1);
theta = flume.foil3.pitch_pos(1:ds:endingstep); % hydrofoil's geometric angle of attack
heave = flume.foil3.heave_pos(1:ds:endingstep); % hydrofoil's heave position
