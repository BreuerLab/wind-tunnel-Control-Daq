function [flume,out,dat,Prof_out_angle,Prof_out,timeStamps,triggerTime] = run_cycle_3rigs_20190712_trapz_vm(freq1,pitch1,heave1_per_chord,pitch3,heave3_per_chord,pitch2,heave2_per_chord, phase12,phase13, number_of_cycles,phi)
...
% Added output: timeStamps,triggerTime (Gali and Varghese, 05/10/2019)

% h = msgbox('Check if clamps are on!','Warning','warn');
% uiwait(h)

%     newobjs = instrfind;
%     delete(newobjs);
%     led = serial('COM14','BaudRate',9600);
%     fopen(led);
%     fprintf(led,'%s',1);
...
% phi = 90;
global s last_out run_num flume_hertz Wbias Gbias chord span foil_shape Temperature foil_separation  pos Wall_distance_left Wall_distance_right Number_of_foils pitch_bias fname beta

%beta = input('0.001 for pure sine, and up to 4 for sharp trapezoidal')
...
[p_prof3,h_prof3] = single_cycle_wallace_trapz_vm([freq2, pitch2, heave2,phi], phase13,0,s.Rate);
% -(last_out(2)+0.888888888888889)*360/5
[p_prof_trans3,h_prof_trans3] = prof_transition_rig2_trapz_vm([freq2,last_pos(5:6)], phase13,0,[freq2, pitch2, heave2,phi],phase13,0,s.Rate);
[p_prof_trans4,h_prof_trans4] = prof_transition_rig2_trapz_vm([freq2, pitch2, heave2,phi],phase13,0,[freq2,0, 0,90],phase13,0,s.Rate);
rate = s.Rate;
...
asd1 = size(prof_trans3);
asd2 = size(prof3);
asd3 = size(prof_trans4);
...
Prof_out_angle = [Prof1  Prof2 Prof3];
% move_new_pos(Prof(1,:));
...
% prepare(s)
T(1,:) = clock;
[dat,timeStamps,triggerTime] = startForeground(s);
