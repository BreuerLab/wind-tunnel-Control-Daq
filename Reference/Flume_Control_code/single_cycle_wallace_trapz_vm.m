function [p_prof, h_prof, t] = single_cycle_wallace_trapz_vm(param, phase1,pitch_bias, rate)
% param = [freq (Hz), p_amp (degs), h_amp (m), A1,B1, C1, D1]  8/27/2014
% -Michael
global chord beta betah
new_param(1:12) = 0;
...
%% This part for trapezoidal pitch profile
 p_prof=pitch_bias*ones(length(t),1)+ new_param(2)*tanh(beta*sin(2*pi*new_param(1).*t +pi*new_param(4)/180+phase_p))'/tanh(beta)...
...
%% This part for pure sinusoidal pitch profile
...
%% This part for trapezoidal heave profile
if betah > 0
h_prof=new_param(3)*tanh(betah*sin(2*pi*new_param(1).*t+phase1+phase_h))'/tanh(betah) ...
    +new_param(7)*sin(3*2*pi*new_param(1)*t+phase1+phase_h)' + new_param(8)*cos(3*2*pi*new_param(1)*t+phase1+phase_h)'...
    +new_param(11)*sin(5*2*pi*new_param(1)*t+phase1+phase_h)' + new_param(12)*cos(5*2*pi*new_param(1)*t+phase1+phase_h)';
else
    h_prof=new_param(3)*asin(-betah*sin(2*pi*new_param(1).*t + +phase1+phase_h))'/asin(-betah) ...
    +new_param(7)*sin(3*2*pi*new_param(1)*t+phase1+phase_h)' + new_param(8)*cos(3*2*pi*new_param(1)*t+phase1+phase_h)'...
    +new_param(11)*sin(5*2*pi*new_param(1)*t+phase1+phase_h)' + new_param(12)*cos(5*2*pi*new_param(1)*t+phase1+phase_h)';


end
%      Pitch = new_param(3)*asin(-betah*sin(2*pi*new_param(1).*t + +phase1+phase_h))'/asin(-betah);
