...
function [t,Ux, Uy, Uz,v,temp] = vector_2_mat(T_start,Length)
try
...
%     tstart(ii,1) = str2double(fnames(ii).name(7:20));
    tstartnum(ii,1) = datenum(fnames(ii).name(7:20),'yyyymmddHHMMSS');
end
...
if isempty(t_ind)
    t_ind = numel(tstart);
    disp('using last saved vector data')
end
t_ind = [t_ind(1)-1; t_ind];


    
fid = load(['C:\Users\ControlSystem\Documents\vertfoil\Experiments\Vector\',fnames(t_ind(1)).name]);
...
%     Ux = ginput('Error reading Vector data, Manually enter average Ux:  ');
disp('Error reading Vector data, reading last file.')
    fnames = dir(['C:\Users\ControlSystem\Documents\vertfoil\Experiments\Vector\','*.vea']);
